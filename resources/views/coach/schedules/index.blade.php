...
@section('content')
@if (! $coach)
    <div class="alert alert-warning">Profil pembina belum ditautkan oleh admin. Anda belum bisa mengelola jadwal.</div>
@else
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5>Jadwal Ekstrakurikuler Dibina</h5>
            <a href="{{ route('coach.schedules.create') }}" class="btn btn-primary btn-sm"><i class="feather icon-plus"></i> Tambah Jadwal</a>
        </div>
        <div class="card-body">
            <form method="GET" class="row mb-3">
                ...
                <div class="col-md-3 mb-2">
                    <select name="extracurricular_id" class="form-control">
                        <option value="">Semua eskul binaan</option>
            ...
            @if ($coach->extracurriculars->isEmpty())
                <x-empty-state message="Belum ada eskul yang dibina. Admin perlu mengatur data eskul terlebih dahulu." />
            @elseif ($schedules->isEmpty())
                <x-empty-state message="Belum ada jadwal untuk eskul binaan Anda." />
            @else
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="action-row justify-content-end">
                                            <a href="{{ route('coach.schedules.edit', $schedule) }}" class="btn btn-info btn-sm"><i class="feather icon-edit"></i></a>
                                            <form method="POST" action="{{ route('coach.schedules.destroy', $schedule) }}" onsubmit="return confirm('Hapus jadwal ini?')">
                                                @csrf
                                                @method('DELETE')
