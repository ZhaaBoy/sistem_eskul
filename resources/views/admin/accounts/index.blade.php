...
@section('content')
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5>Akun Pengguna</h5>
            <a href="{{ route('admin.accounts.create') }}" class="btn btn-primary btn-sm"><i class="feather icon-plus"></i>
                Tambah Akun</a>
        </div>
        <div class="card-body">
            <form method="GET" class="row mb-3">
                ...
                <div class="col-md-2 mb-2">
                    <button class="btn btn-secondary btn-block" type="submit"><i class="feather icon-search"></i>
                        Filter</button>
                </div>
            </form>
                                    ...
                                    <td>
                                        <div class="action-row justify-content-end">
                                            <a href="{{ route('admin.accounts.edit', $account) }}"
                                                class="btn btn-info btn-sm"><i class="feather icon-edit"></i></a>
                                            <form method="POST"
                                                action="{{ route('admin.accounts.toggle-status', $account) }}">
                                                @csrf
                                                <button class="btn btn-warning btn-sm" type="submit"><i
                                                        class="feather icon-power"></i></button>
                                            </form>
                                            {{-- <button class="btn btn-secondary btn-sm" type="button" data-toggle="collapse"
                                                data-target="#reset-{{ $account->id }}"><i
                                                    class="feather icon-key"></i></button> --}}
                                        </div>
